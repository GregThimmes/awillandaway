...
function documentReadyInit() {
	...
	if ($().localScroll) {
		$('.top-nav > ul, .mainmenu_side_wrapper > ul, .scroll-link,  .comments-link').localScroll({
			duration:900,
...
function windowLoadInit() {
	...
	if ($().owlCarousel) {
		$('.owl-carousel, .uws-products.carousel').each(function() {
			...
			var loop = data.loop ? data.loop : false,
				...
				navPrev = data.navPrev ? data.navPrev : '<i class="rt-icon2-chevron-thin-left">',
				navNext = data.navNext ? data.navNext : '<i class="rt-icon2-chevron-thin-right">',
			...
			if ( $carousel.hasClass('uws-products') ) {
				$carousel = $carousel.find('.products').addClass('owl-carousel owl-theme');
			}
			...
			if($carousel.hasClass('testimonials-slider3') ) {
	...
	if ( document.querySelector('.fw-flash-message, .form-error') ) {
